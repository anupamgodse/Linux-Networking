...
do
  ...
  echo $timestamp,$la1,$la5,$la15 >> $HOME/load.csv
  ...
  if [ $( echo "$la1 > $X" | bc ) -eq 1 ]; then
    echo $timestamp,"HIGH CPU usage",$la1,$la5,$la15 >> $HOME/load_alerts.csv
  fi

  if [ $( echo "$la5 > $Y" | bc ) -eq 1 ] && [ $increasing -eq 1 ]; then
    echo $timestamp,"Very HIGH CPU usage",$la1,$la5,$la15 >> $HOME/load_alerts.csv
  fi
