...
do
  ...
  echo $timestamp $load_avgs >> load.csv
  ...
  echo $la1 $la5 $la15 $last_la1

  if [ $(echo "$la1 > $last_la1" | bc) -eq 1 ] && [ $last_la1 -ne -1 ]; then
    increasing=1
  else
    increasing=0
  fi
  ...
  if [ $(echo "$la1 > $X" | bc) -eq 1 ]; then
    echo $timestamp "HIGH CPU usage" $load_avgs >> load_alerts.csv
  fi

  if [ $(echo "$la5 > $Y" | bc) -eq 1 ] && [ $increasing -eq 1 ]; then
    echo $timestamp "Very HIGH CPU usage" $load_avgs >> load_alerts.csv
  fi
